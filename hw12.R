...
bike_sales_y %>%
  ...
  # Replace NA (first year) with current year sales
  mutate(
...
bike_sales_y %>%
  ...
  geom_point(aes(color = sales), size = 5) + 
  geom_line(aes(color = sales), linewidth = 2) + 
  
  geom_smooth(
    method = "lm",
    formula = y ~ x,
    se = FALSE,
    color = "blue"   # fixed color for trend line
  ) +
...
pct_sales_by_customer_tbl %>% 
  ...
  geom_text(aes(label = scales::percent(pct, accuracy = 0.01)), size = 3) +
